...
FASTGRAPHRAG_CHUNK_SIZE = 200   # v0.5.0: 100→200 (ベンチマーク結果: 構築時間-34%)
FASTGRAPHRAG_CHUNK_OVERLAP = 20

# キャッシュ設定（環境変数で上書き可能）
CACHE_TTL_HOURS = int(os.environ.get("GRAPHRAG_CACHE_TTL_HOURS", "24"))
SIMILARITY_THRESHOLD = float(os.environ.get("GRAPHRAG_CACHE_SIMILARITY", "0.60"))
CACHE_MAX_ENTRIES = int(os.environ.get("GRAPHRAG_CACHE_MAX_ENTRIES", "50"))
...
class IndexCache:
    ...
    マルチプロセス安全:
      - ファイルロック (fcntl.flock) によるマニフェスト排他制御
      - 読み取りは LOCK_SH、書き込みは LOCK_EX

    LRU エビクション:
      - max_entries を超えた場合、最も古い last_accessed のエントリを削除

    使用頻度追跡:
      - access_count / last_accessed を記録し、キャッシュ統計を提供
    """
    ...
    def __init__(self, cache_dir: Path = DEFAULT_CACHE_DIR,
        ...
        self._lock_file = self.cache_dir / ".manifest.lock"

    # ── ファイルロック ─────────────────────────────────────
    def _acquire_lock(self, exclusive: bool = False):
        """マニフェストのファイルロックを取得"""
        fd = open(self._lock_file, "a+")
    ...
    def find_cached_index(self, query_embedding: list[float]) -> dict | None:
        """類似クエリのキャッシュ済みインデックスを検索 (排他ロック: last_accessed 更新)"""
        ...
        try:
            ...
            if best_entry:
                # 使用頻度を更新
                best_entry["last_accessed"] = now.isoformat()
                best_entry["access_count"] = best_entry.get("access_count", 0) + 1
                self._save_manifest(manifest)
                print(f"  キャッシュヒット! 類似度: {best_similarity:.3f} "
                      f"(アクセス #{best_entry['access_count']})")
    ...
    # ── LRU エビクション ──────────────────────────────────
    def _evict_lru(self, manifest: dict):
        """max_entries を超えた場合、最も古い last_accessed のエントリを削除"""
        entries = manifest["entries"]
        if len(entries) <= self.max_entries:
            return

        # last_accessed でソート（古い順）
        entries.sort(
            key=lambda e: e.get("last_accessed", e.get("created_at", "")),
        )
        while len(entries) > self.max_entries:
            evicted = entries.pop(0)
            idx_path = Path(evicted["index_path"])
            if idx_path.exists():
                shutil.rmtree(idx_path, ignore_errors=True)
            print(f"  キャッシュ LRU エビクション: {evicted['query'][:40]}...")

    # ── キャッシュ登録 ─────────────────────────────────────
    def register_index(self, query: str, query_embedding: list[float],
                       index_path: Path, paper_ids: list[str]):
        """新しいインデックスをキャッシュに登録 (排他ロック)"""
        fd = self._acquire_lock(exclusive=True)
        try:
            manifest = self._load_manifest()
